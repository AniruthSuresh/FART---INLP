...
class HybridFNetBartEncoder(BartPreTrainedModel):
    """
    Hybrid Encoder: Mix of FNet (FFT) and BART Attention layers.
    """
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None, fnet_ratio=0.5):
        ...
        # --- Hybrid: alternate FNet and Attention layers ---
        total_layers = config.encoder_layers
        n_fnet = int(total_layers * fnet_ratio)
        n_attn = total_layers - n_fnet
        # Alternate: [FNet, Attn, FNet, Attn, ...]
        layers = []
        for i in range(total_layers):
            if i % 2 == 0 and n_fnet > 0:
                layers.append(FNetLayer(config))
                n_fnet -= 1
            else:
    ...
    def forward(
        ...
        for idx, layer_module in enumerate(self.layers):
            if output_hidden_states:
                encoder_states = encoder_states + (hidden_states,)
            # FNetLayer does not use attention_mask, BartEncoderLayer does
            if isinstance(layer_module, FNetLayer):
                hidden_states = layer_module(hidden_states)
            else:
...
hybrid_encoder = HybridFNetBartEncoder(config=config, embed_tokens=shared_embeddings, fnet_ratio=0.1) 
model.model.encoder = hybrid_encoder
...
wandb.init(project="jedi-configs", name="BART-fnet-ratio-0.2" ,
            config={
